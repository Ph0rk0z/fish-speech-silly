...
def inference(req: ServeTTSRequest):
    ...
    if idstr is not None:
        ref_folder = Path("references") / idstr # / req.emotion  # Assuming you still want to use 'emotion'

        if not ref_folder.exists():
            raise HTTPException(
                HTTPStatus.BAD_REQUEST,
                content=f"Reference audio folder not found for ID: {idstr}", #, emotion: {req.emotion}",
            )

        wav_files = list(ref_folder.glob("*.wav"))
        lab_files = list(ref_folder.glob("*.lab"))

        if not wav_files or not lab_files:
            raise HTTPException(
                HTTPStatus.BAD_REQUEST,
                content=f"No .wav or .lab files found for ID: {idstr}", #, emotion: {req.emotion}",
            )
        ...
        wav_file, lab_file = wav_files[0], lab_files[0]
        logger.info(f"Loading reference audio from: {wav_file}")

#        prompt_tokens = [
#            encode_reference(
#                decoder_model=decoder_model,
#                reference_audio=str(wav_file),
#                enable_reference_audio=True,
#            )
#        ]
#        prompt_texts = [read_ref_text(str(lab_file))]
        ...
        for wav_file, lab_file in zip(wav_files, lab_files):
            ...
            prompt_texts.append(read_ref_text(str(lab_file)))
...
app = Kui(
    ...
    factory_class=FactoryClass(http=JSONRequest),
    cors_config={},
)
